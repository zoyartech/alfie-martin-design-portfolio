      ...
      <motion.div
        ...
        <Link to={project.link ? createPageUrl(project.link) : '#'} className="flex flex-col md:flex-row w-full h-full relative" style={{ backgroundColor: project.bgColor || '#fee2e2' }}>
          ...
          <div className="p-8 w-full md:w-1/2 h-1/2 md:h-full md:p-16 flex flex-col">
            ...
            <div className="bg-cyan-800 text-white mt-auto mb-8 pb-8 grid grid-cols-2 gap-6 border-b hidden md:grid border-gray-100">
                {project.stats.map((stat) =>
              <div key={stat.label}>
                    <p className="text-slate-50 mb-1 text-2xl font-light md:text-3xl">{stat.value}</p>
                    <p className="text-slate-50 text-sm font-medium">{stat.label}</p>
                  </div>
              )}
              </div>
            }
