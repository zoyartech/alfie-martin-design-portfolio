      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 relative z-10">
        <div className="bg-slate-50 text-slate-950 flex items-center justify-between h-20">
          ...
          <div className="text-slate-950 hidden md:flex items-center gap-10">
            ...
            <Link
              to={createPageUrl("Insights")} className="text-slate-950 text-xs font-medium tracking-[0.15em] relative transition-colors group hover:text-white/80">
              
              INSIGHTS
              <span className={`absolute bottom-0 left-0 h-[2px] transition-all duration-300 ${path === 'Home' ? 'bg-white w-0 group-hover:w-full' : path === 'Insights' ? 'bg-black w-full' : 'bg-black w-0 group-hover:w-full'}`}></span>
            </Link>
            <Link
              to={createPageUrl("Writing")} className="text-slate-950 text-xs font-medium tracking-[0.15em] relative transition-colors group hover:text-white/80">
